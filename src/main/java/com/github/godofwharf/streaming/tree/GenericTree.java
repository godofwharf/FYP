package com.github.godofwharf.streaming.tree;
...
public class GenericTree<T> {
    ...
    public GenericTreeNode<Integer> bfs() {

        int k = 4;
        // BFS uses Queue data structure
        Queue<GenericTreeNode<Integer>> queue = new LinkedList<GenericTreeNode<Integer>>();
        queue.add((GenericTreeNode<Integer>) getRoot());
        //printNode(this.rootNode);
        //rootNode.visited = true;
        while (!queue.isEmpty()) {
            GenericTreeNode<Integer> node = (GenericTreeNode<Integer>) queue.remove();
            if (node.getNumberOfChildren() < k) {
                return node;
            }
            //GenericTreeNode<Integer> child=null;
